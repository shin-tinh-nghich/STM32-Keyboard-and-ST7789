...
void Arduino_ST7789::init(uint16_t width, uint16_t height) 
{
  ...
  SPI_2.beginTransaction(SPISettings(SPI_FREQ, MSBFIRST, SPI_MODE3, DATA_SIZE_8BIT));
  ...
  setRotation(2);
  SPI_2.setDataSize(DATA_SIZE_16BIT);
}
...
{
  SPI_2.setDataSize(DATA_SIZE_8BIT);
...
void Arduino_ST7789::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) 
{
  ...
  DC_COMMAND; SPI_2.write(ST7789_CASET);
  DC_DATA; SPI_2.write(x0); SPI_2.write(x1);

  DC_COMMAND; SPI_2.write(ST7789_RASET);
  DC_DATA; SPI_2.write(y0); SPI_2.write(y1);

  DC_COMMAND; SPI_2.write(ST7789_RAMWR);
...
void Arduino_ST7789::drawPixel(int16_t x, int16_t y, uint16_t color) 
{
  ...
  CS_ACTIVE;
  DC_COMMAND; SPI_2.write(ST7789_CASET);
  DC_DATA; SPI_2.write(x); SPI_2.write(x+1);
  DC_COMMAND; SPI_2.write(ST7789_RASET);
  DC_DATA; SPI_2.write(y); SPI_2.write(y+1);
  DC_COMMAND; SPI_2.write(ST7789_RAMWR);
  DC_DATA; SPI_2.write(color);
  CS_IDLE;
}
...
void Arduino_ST7789::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) 
{
  ...
  if(num>DMA_MIN) {
    while(num>DMA_MAX ) {
      num -= DMA_MAX;
      SPI_2.dmaSend(dmaBuf, DMA_MAX, 0);
    }
    SPI_2.dmaSend(dmaBuf, num, 0);
  } else 
#endif
  SPI_2.write(color, num);
